...
// ObjectBackedDirectoryFactory is a factory type for creating read-only
// virtual file system directories that contain the literal contents of
// directory objects that have been written to storage.
//
// This cannot be used on workers to materialize input roots, as build
// actions assume that input roots are writable. However, it is
// sufficient for exploring the files generated by a build after the
// build completes.
...
// NewObjectBackedDirectoryFactory creates an
// ObjectBackedDirectoryFactory that reads directory and leaves objects
// from a given storage backend.
...
// will reflect the contents of a specific directory in storage when
// accessed.
